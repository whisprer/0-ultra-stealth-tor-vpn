...
pub async fn maybe_tick(cfg: &Config, state_dir: &Path) -> Result<()> {
    ...
    let mut st: HopState = if let Ok(b) = fs::read(&state_path).await {
        serde_json::from_slice(&b).unwrap_or(HopState { idx: 0, next_epoch_ms: 0, order: vec![] })
    } else { HopState { idx: 0, next_epoch_ms: 0, order: vec![] } };

    if st.next_epoch_ms == 0 {
        st.order = (0..cfg.hop.sequence.len()).collect();
        if cfg.hop.randomize { st.order.shuffle(&mut rand::thread_rng()); }
        st.idx = 0;
    ...
    if now_ms() >= st.next_epoch_ms {
        ...
        if idx >= cfg.hop.sequence.len() {
            if cfg.hop.loop_forever { idx = 0; st.order = (0..cfg.hop.sequence.len()).collect(); if cfg.hop.randomize { st.order.shuffle(&mut rand::thread_rng()); } } else {
                st.idx = idx; st.next_epoch_ms = u64::MAX;
                fs::write(&state_path, serde_json::to_vec_pretty(&st)?).await?; return Ok(());
            }
        }
...
async fn apply_item(cfg: &Config, state_dir: &Path, item: &HopItem) -> Result<()> {
    ...
    match &item.proxy {
        ...
        Some(url) => {
            if url.starts_with("socks5://") { let addr = url.trim_start_matches("socks5://"); apply_proxy(&mut ctl, "socks5", addr, None, None).await?; }
            else if url.starts_with("https://") { let addr = url.trim_start_matches("https://"); apply_proxy(&mut ctl, "https", addr, None, None).await?; }
        }
    }
